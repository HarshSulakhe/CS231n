...
def content_loss(content_weight, content_current, content_original):
    ...
    C,H,W = content_current.squeeze(0).size()
    F = content_current.squeeze(0).view(C,-1)
    P = content_original.squeeze(0).view(C,-1)
    loss = torch.sum((F-P)**2)*content_weight
    return loss
    pass
...
def gram_matrix(features, normalize=True):
    ...
    # print(N,C,H,W)
    # print(gram.size(),F.transpose(1,2).size())
    if normalize:
        gram/=(H*W*C)
    return gram
    pass
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        gram_sty = style_targets[i]
        weight = style_weights[i]
        style_loss += weight*(torch.sum((gram_cur - gram_sty)**2))
    return style_loss
    pass
...
def tv_loss(img, tv_weight):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N,C,H,W = img.size()
    h_img = torch.zeros_like(img)
    w_img = torch.zeros_like(img)
    loss = 0
    h_img[:,:,1:H,:] = img[:,:,0:H-1,:]
    h_img[:,:,0,:] = img[:,:,0,:]
    w_img[:,:,:,1:W] = img[:,:,:,0:W-1]
    w_img[:,:,:,0] = img[:,:,:,0]
    loss += tv_weight*(torch.sum((img-h_img)**2) + torch.sum((img-w_img)**2))
    return loss
    pass
