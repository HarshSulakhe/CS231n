...
def compute_saliency_maps(X, y, model):
    ...
    outputs = model(X)
    outputs = outputs.gather(1,y.view(-1,1)).squeeze()
    outputs.backward(torch.FloatTensor([1.0]*outputs.shape[0]))
    # outputs.backward()
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        ...
        if torch.argmax(output) == target_y:
            break
        weights = torch.zeros(output.size())
        weights[:,target_y] = 1
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    output = model(img)
    score = output[:,target_y] - (l2_reg*torch.norm(img))
    score.backward()
