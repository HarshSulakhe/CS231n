...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        vectors, cache_embed = word_embedding_forward(captions_in,W_embed)

        forward_net = {'lstm': lstm_forward, 'rnn': rnn_forward}[self.cell_type]
        hidden_state_vectors,cache_net = forward_net(vectors,h0,Wx,Wh,b)

        scores,cache_temporal = temporal_affine_forward(hidden_state_vectors,W_vocab,b_vocab)

        loss, dout = temporal_softmax_loss(scores,captions_out,mask)

                            ### BACKWARD PASS ###

        dout,dW_vocab,db_vocab = temporal_affine_backward(dout,cache_temporal)

        backward_net = {'lstm': lstm_backward, 'rnn': rnn_backward}[self.cell_type]
        dout,dh0,dWx,dWh,db = backward_net(dout,cache_net)
