...
def discriminator_loss(logits_real, logits_fake):
    ...
    # print(N)
    y_real = torch.ones((N,)).type(torch.cuda.FloatTensor)

    y_fake = torch.zeros((N,)).type(torch.cuda.FloatTensor)
...
def generator_loss(logits_fake):
    ...
    y_fake = torch.ones((N,)).type(torch.cuda.FloatTensor)
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, loader_train, show_every=250,
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            logits_real = D((2* (real_data - 0.5)).view(batch_size,1,28,28)).type(dtype)
