...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h_ = x.dot(Wx) + prev_h.dot(Wh) + b
    next_h = np.tanh(next_h_)
    cache = x,Wx,Wh,prev_h,next_h_
    pass
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x,Wx,Wh,prev_h,next_h_ = cache
    dnext_h_ = dnext_h*(1/np.cosh(next_h_))**2 ## (N,H)
    db = np.sum(dnext_h_,axis = 0) ## (H,)
    dx = np.dot(dnext_h_,Wx.T) ## (N,D)
    dWx = np.dot(x.T,dnext_h_) ## (D,H)
    dWh = np.dot(prev_h.T,dnext_h_) ## (H,H)
    dprev_h = np.dot(dnext_h_,Wh.T) ## (N,H)
    pass
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for vector in range(T):
        next_h,c = rnn_step_forward(x[:,vector,:],next_h,Wx,Wh,b)
        forward_cache[vector] = c
        h[:,vector,:] = next_h
...
def rnn_backward(dh, cache):
    ...
    for vector in reversed(range(T)):
        dh_ = dh[:,vector,:] + dprev_h
        dx_,dprev_h,dWx_,dWh_,db_ = rnn_step_backward(dh_,forward_cache[vector])
        dWx += dWx_
        dWh += dWh_
        db += db_
        dx[:,vector,:] = dx_
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # gradients of next_c from next_h get added to dnext_c
    dc_  = dnext_h*o*(1-np.tanh(next_c)**2)
    dc_ += dnext_c
    dprev_c = dc_*f
