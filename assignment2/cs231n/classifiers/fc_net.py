...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            x,cache = affine_bn_relu_forward(x,w,b,gamma,beta,bn_param,self.normalization,self.use_dropout,self.dropout_param)
            caches.append(cache)
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss,dout = softmax_loss(scores,y) #calculate softmax loss
        for i in range(self.num_layers): # add regularization loss
            w = self.params['W'+str(i+1)]
            loss += self.reg*0.5*np.sum(w*w)

        # complete first backward pass
        ...
        for i in range(self.num_layers-2,-1,-1): # go in reverse order
            dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, caches[i], self.normalization,self.use_dropout)
...
## Helper functions
def affine_bn_relu_forward(x,w,b,gamma,beta,bn_param,normalization,dropout,do_param):
...
def affine_bn_relu_backward(dout,cache,normalization,dropout):
    ...
    if normalization == 'batchnorm':
        drelu,dgamma,dbeta = batchnorm_backward_alt(drelu,bn_cache)
    elif normalization == 'layernorm':
